          ...
          <tbody className="divide-y divide-slate-100">
            {rows.map((row) => (
              <tr key={row.productId}>
                <td className="py-3 font-medium text-slate-900">{row.sku}</td>
                <td className="py-3">{row.productName}</td>
                <td className="py-3">
                  <Badge variant={riskVariant(row.urgency)}>{row.urgency}</Badge>
                </td>
                <td className="py-3">{row.recommendedQuantity}</td>
                <td className="py-3">{row.projectedStockoutDate}</td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  return (
    <Card className="overflow-hidden">
      ...
      <div className="space-y-4">
        {rows.map((row) => (
          <div key={row.supplierId} className="rounded-2xl border border-slate-100 p-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                <div className="font-medium text-slate-900">{row.supplierName}</div>
                <div className="mt-1 text-sm text-slate-500">{row.recommendation}</div>
              </div>
...
export function PredictionStack({
  ...
  return (
    <Card>
      ...
      <div className="space-y-4">
        {rows.map((row) => (
          <div key={"shipmentId" in row ? row.shipmentId : row.assetId} className="rounded-2xl bg-slate-50 p-4">
            <div className="flex items-center justify-between gap-3">
